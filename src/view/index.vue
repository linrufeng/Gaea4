 <template>
  ...
  <div v-show="!reachEnd"
    ...
    <div class="weekbar">
      ...
      <span class="lefttime">{{ weekList[curDay]&&weekList[curDay].year}}年{{weekList[curDay]&&weekList[curDay].month}}月</span>
      <span class="righttime">
        <span class="leftdate">
          <span class="startdate">{{weekList[curDay]&&weekList[curDay].startDate}}</span>
          <span class="transarrow"></span>
          <span class="enddate">{{weekList[curDay]&&weekList[curDay].endDate}}</span>
        </span>
...
export default class Index extends Vue {
  ...
  async getAllDate() {
    ...
    let weekList = _that.weekList;
    let selectIndex = _that.curDay;
    if(!weekList[selectIndex]){
      return
    }   
    const sever = await _that.axios.get(getWeekDataByType,{
        params:{
          userName:usethird_name,
          startDate:weekList[selectIndex].startNum,
          endDate:weekList[selectIndex].endNum
        }
      });      
  ...
  mounted() {
     let selectDay:string = this.$route.query.select || '0';
     this.curDay = Number(selectDay);      
  }
}
</script>
